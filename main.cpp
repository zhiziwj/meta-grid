...
result back_test(std::vector<kline> data,strategy s){
	...
	result ans;
	double base_price=data[0].o;
	for(int i=0;i<data.size();i++){
		double price=data[i].c/base_price;
		double value=0.0,ratio;
		for(int j=0;j<state;j++){
			if(price<=s.p[j+1]){
				ratio=(price-s.p[j])/(s.p[j+1]-s.p[j]);
				value=s.s[j]+ratio*(s.s[j+1]-s.s[j]);
				break;
			}
		}
		double t_shares=value/price;
...
double check(std::vector<double> line){
	...
	double R=pow(1.0+total,1.0/time_unit)-1.0;
...
SA_state SA(std::vector<kline> data){
	...
	double best=cur;
	double T=1000,speed=0.99;
	int num_time=50;
	while(T>1e-4){
		for(int i=1;i<=num_time;i++){
			SA_state new_state=cur_state;
			int rd=rand_dim();
			new_state.u[rd]+=(rand01()-0.5)*T*0.005;
			new_state.u[rd]=std::max(new_state.u[rd],(double)1e-5);
			new_state.t[rd]+=(rand01()-0.5)*T*0.005;
